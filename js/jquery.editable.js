...
$.editableFactory = {
	'text': {
		toEditable: function($this,options){
			current = $this.data('editable.current');
			if (!(current==='<input>')) {
				Spank.currentlyClickedPlaylistItemName = $this.data('editable.current');
			}
			$('<input/>').appendTo($this)
						 .val(Spank.currentlyClickedPlaylistItemName);
		},
		getValue: function($this,options) {
			current = $this.children().val();
			if (current==='<input>') {
				current = Spank.currentlyClickedPlaylistItemName;
			}
			return current;
		}
	},
