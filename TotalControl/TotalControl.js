...
(function( $ ) {
  ...
  $.fn.totalControl = function(options) {
     ...
     return this.each(function() {
		 ...
		 $('head').append('<link rel="stylesheet" href="/TotalControl/css/' + options.style + '" type="text/css" />');
		 var playerHtml = '';
		 var totalSongs = 0;
		 var useFlash = "no";
		 ...
		 // Get initial list of songs
      	 thisPlayer.find("li").each(function () {
			...
			{
				var artist = '<div class="total-artist">@ARTIST@</div>'.replace("@ARTIST@", thisArtist);
				var frag = '<div onclick="" class="total-row can-play" @ARTWORK@><div type="checkbox" onclick="" class="total-checked"></div><div class="total-not-playing"></div><div class="total-title" src="@URL@">@TITLE@</div>'+artist+'<div style="clear:both;"></div></div>';
				frag = frag.replace("@TITLE@", thisTitle).replace("@URL@", thisSrc).replace("@ARTWORK@",artwork);
				playerHtml += frag;
				totalSongs++;
			}
			...
			{		var newClickTime = new Date().getTime();
					...
					if(timeDelta < 350)
					{
						if(useFlash == "no")
						{
							...
							currentRow = $(this);
							songPlay(newSrc);
							$(".total-position-scrubber").css("background-image", "-webkit-linear-gradient(0, rgb(0,255,30) 0%, rgb(250,42,42) 0%)");
							htmlSound.play();
							function updateSeekable() {
								var perc = (this.buffered.end(0)/this.duration)*100,
									gradient = "-webkit-linear-gradient(0, rgb(0,255,30) @%, rgb(250,42,42) 0%)".replace("@",perc);
								$(".total-position-scrubber").css("background-image",gradient);
							}
							htmlSound.addEventListener("progress", updateSeekable, false);
							htmlSound.addEventListener("timeupdate", function() {
								var newVolume = thisPlayer.find( ".total-volume-slider" ).slider("option", "value");
								htmlSound.volume = newVolume;
